from standard_dicts import update_dict_neuron
from ast import If
...
def all_dense_layers_gen(
    ...
    Returns:
        layer_dict_list(list): Lista de dicionários. Será usada pela função que gera o topo. Pois o mesmo deverá coletar a lista de IO (Inputs & Outputs) de cada camada e fazer o 'PORT MAP' entre as mesmas.
    """
    ...
    # layer_dict_list = [None]*number_of_layers
    layer_dict_list = [[None] for i in range(number_of_layers)]
    ...
    for i in range(0, len(layer_dict_list)):
        ...
        if (i == (len(layer_dict_list)-1)):  # na última camada, atribui dicionário base softmax
            layer_dict_list[i] = copy.deepcopy(base_dict_softmax_layer)
    ...
    layer_dict_list[0] = layer_dict_gen_base(base_dict=layer_dict_list[0],
    ...
    for i in range(1, len(layer_dict_list)):
        if (gen_dead_neurons == True):  # com neuronios mortos
            neurons_number = np.max(Layer_Neurons_number_list)
        else:
            neurons_number = Layer_Neurons_number_list[i]

        layer_dict_list[i] = layer_dict_gen_base(base_dict=layer_dict_list[i],
                                                 Inputs_number=layer_dict_list[i -
                                                                               1]['Neurons_number'],
                                                 bits=bits,
                                                 IO_type=IO_type_str,  # 1= signed || 0= unsigned
                                                 Neurons_number=neurons_number,
    ...
    for i in range(0, len(layer_dict_list)):
        ...
        if (gen_dead_neurons == True):  # caso queiramos gerar neurônios mortos
            ...
            #     layer_dict_arg = layer_dict_list[i],
            #     Layer_num = layer_dict_list[i]['Layer_num'],
    ...
    #         for i in range(0, len(layer_dict_list)):
    #             print(
    #                 f"all_dense_layers_gen()  -> dict[{i}] -> Neuron num_inputs: {layer_dict_list[i]['Neuron_arch']['Inputs_number']()}")

    #             print(
    #                 f"all_dense_layers_gen()  -> dict[{i}] -> Layer  num_inputs: {layer_dict_list[i]['Inputs_number']}")
    #     except:
    #         try:
    #             for i in range(0, len(layer_dict_list)):
    #                 print(
