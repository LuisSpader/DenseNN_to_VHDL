...
def Neuron_Gen_from_dict2(
    ...
    output_name = dict_list_exceptNone(
        dict_slice=layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']['OUT'], return_value_or_key='value', is_list=False)
    # output_name = [['IO_out'], ['W_out : OUT signed(BITS - 1 DOWNTO 0);']]
    # ! problema aqui que não está totalmente automatizado (deveria encontrar alguma string padrão, tipo 'IO')
    output_name = output_name[0][0]
    # output_name = 'IO_out'
    ...
    neuron_type = fx_activation  # ReLU, Leaky, Softmax
    leaky_attenuation = layer_dict_list[i]['Neuron_arch']['Activation_fx']['Leaky_ReLU']['Leaky_attenuation']
    # leaky_attenuation = int
    # IO_type = IO_type
    input_mem_bits = layer_dict_list[i]['Neuron_arch']['Activation_fx']['Sigmoid']['Memory']['bits_mem']
    ...
    MAC_name = vhd_name(
        ...
        Include_MAC_type=Include_MAC_type,
        mult_number=find_True_dict_split(
            split_str='-', dict=layer_dict_list[i]['Neuron_arch']['Multiplier'], position=0),
        # Versão do modelo de multiplicador (pois podem existir melhorias em um modelo). Exemplo: mult_version = 4 --> v4
        mult_version=0,
        adder_number=find_True_dict_split(
            split_str='-', dict=layer_dict_list[i]['Neuron_arch']['Adder'], position=0),     # análogo ao 'mult_number'
        adder_version=0)
    ...
    mac_entity_txt = entity(
        name=MAC_name,
        bits=layer_dict_list[i]['Neuron_arch']['Bit_WIDTH'],
        num_inputs=layer_dict_list[i]['Neuron_arch']['Inputs_number'],
        IO_dict_list=[
    ...
    W_in = layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']['IN']['manual'][0]
    # W_in = 'W_in : IN signed(BITS - 1 DOWNTO 0);'
    ...
    MAC_component = entity_to_component(
        entity_text=MAC_component,
        word=layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']['IN']['manual'][0],
        word_subs=W_in_subs)
    # mesma coisa que mac_entity_txt porém trocando o tamanho da entrada de pesos (todos de uma vez devem entrar no MAC, vindo dos shift_regs)
...
''')
        ...
        port_map_shift_reg_No_Wout = ''
# -------------------------------------------------
    entity_leaky_out = entity(f"{neuron_Leaky_name}_out", bits, num_inputs,
                              [layer_dict_list[i]['Neuron_arch']['IO']['shared_IO'],
                               layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']],
...
{PORT_MAP_MAC}
{port_map_shift_reg}
{f"{layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']['OUT']['SIGNED'][0]} <= out_reg_MAC;"}

END behavior;'''
                                )

    entity_leaky = entity(neuron_Leaky_name, bits, num_inputs,
                          [layer_dict_list[i]['Neuron_arch']['IO']['shared_IO'],
                              layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']],
                          remove_dict_items=[
                              'W_out : OUT signed(BITS - 1 DOWNTO 0);'],
...
# -------------------------------------------
    entity_ReLU_out = entity(f"{neuron_ReLU_name}_out", bits, num_inputs,
                             [layer_dict_list[i]['Neuron_arch']['IO']['shared_IO'],
                              layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']],
                             #   remove_dict_items=['W_out : OUT signed(BITS - 1 DOWNTO 0);'],
                             generic=True)
...
END behavior;'''
    ...
    entity_ReLU = entity(neuron_ReLU_name, bits, num_inputs,
                         [layer_dict_list[i]['Neuron_arch']['IO']['shared_IO'],
                          layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']],
                         remove_dict_items=[
                             'W_out : OUT signed(BITS - 1 DOWNTO 0);'],
...
{entity(neuron_Sigmoid_name, bits, num_inputs,[layer_dict_list[i]['Neuron_arch']['IO']['shared_IO'], layer_dict_list[i]['Neuron_arch']['IO']['unique_IO'] ],generic = True)}
...
{PORT_MAP_ROM}
    ...
    {f"{layer_dict_list[i]['Neuron_arch']['IO']['unique_IO']['OUT']['SIGNED'][0]} <= signed(out_ROM_act);"}
...
END behavior;'''
    ...
    print(f"----------->>>>>>>>>>> path_others {path_others}")
    adder_txt_gen(layer_dict_list[i], path_others, create_path_folder=False)
    multiplier_txt_gen(layer_dict_list[i],
