...
def layer_gen(layer_dict_arg: dict, Layer_num: int = 0) -> str:
...
def all_dense_layers_gen(
    ...
    # LAYERS_DICT_LIST.append(layer_dict_list[0])
    ...
    for i in range(1, len(layer_dict_list)):
        ...
        # LAYERS_DICT_LIST.append(layer_dict_list[i])
    ...
    for i in range(0, len(layer_dict_list)):
        ...
        layer_text = layer_gen(
            layer_dict_arg=layer_dict_list[i], Layer_num=layer_dict_list[i]['Layer_num'])

        # LAYERS_DICT_LIST.append(layer_dict_list[i])
        # salvando VHDL
        if (download_vhd == 1):
            with open(f"{path}{layer_dict_list[i]['Layer_name']}.vhd", "w") as writer:
